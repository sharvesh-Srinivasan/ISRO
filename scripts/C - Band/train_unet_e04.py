# ============================================================
# train_unet_e04.py  —  C-band (EOS-04) version
# ------------------------------------------------------------
# KEY DIFFERENCES from train_unet.py (L-band):
#
...
#                       image and destroys contrast.
#                       Run find_thresholds_e04.py to get correct values.
#
#  3. THRESHOLDS     — generate_mask_lib() uses C-band dB boundaries.
#                       L-band used (10.142 / 19.638 / 20.818). C-band
#                       has different absolute backscatter magnitudes
#                       due to shorter wavelength and different scattering
#                       mechanisms, so those L-band values will
#                       mis-classify land cover on C-band data.
#                       Run find_thresholds_e04.py to get correct values.
#
#  4. MODEL SAVES    — saved with _e04 suffix so L-band models are
#                       not overwritten.
# ============================================================
import os
import numpy as np
...
# different backscatter magnitudes, so those L-band values
# clip most C-band pixels to 0 or 255, losing image detail.
#
...
def generate_mask_lib(db_image):
    # ------------------------------------------------------------
    # CHANGE 2: C-BAND CLASSIFICATION THRESHOLDS
    # ------------------------------------------------------------
    # L-band used: Water<=10.142, Land<=19.638, Cropland<=20.818
    # These were derived from L-band backscatter statistics.
    # C-band (EOS-04) has different absolute dB magnitudes due to
    # its shorter wavelength (5.6 cm vs ~23 cm for L-band) and
    # stronger surface scattering — the same L-band boundaries will
    # mis-classify land cover classes on C-band data.
    #
    # ACTION REQUIRED: Run find_thresholds_e04.py on the E04 tiles.
    # It uses K-Means on actual C-band composite values to find the
    # correct boundaries. Paste the printed values below.
    #
    ...
    threshold_water    = [-np.inf, 10.142]   # <-- UPDATE with C-band value from find_thresholds_e04.py
    threshold_land     = [10.142,  19.638]   # <-- UPDATE with C-band value from find_thresholds_e04.py
    threshold_cropland = [19.638,  20.818]   # <-- UPDATE with C-band value from find_thresholds_e04.py
    threshold_mountain = [20.818,  np.inf]   # <-- UPDATE with C-band value from find_thresholds_e04.py
    # (These are currently still the L-band values — they MUST be replaced)
...
def prepare_data():
    ...
    for hh_file, hv_file in zip(hh_files, hv_files):
        ...
        # L-band values (-7, 3.5). The stretch range must match the actual
        # dB dynamic range of the C-band data to preserve image contrast.
        # C_LOWER and C_UPPER are defined at the top of this file.
        ...
        # Generate the mask library for the dB image using C-band thresholds
...
def main():
    ...
    for model_name, model in models.items():
        ...
        # CHANGE 5: _e04 suffix keeps C-band models separate from L-band models
        # L-band saves as: model_u-net.h5 / model_cnn.h5 / model_vision.h5
        # C-band saves as: model_u-net_e04.h5 / model_cnn_e04.h5 / model_vision_e04.h5
        save_name = f'd:/ISRO/Proj/model_{model_name.split()[0].lower()}_e04.h5'
